...
class LoadTestCase(TransactionTestCase):
    ...
    def simulate_user_session(self, username, duration=60):
        ...
        while time.time() - start_time < duration:
            # Simulate user thinking time with longer intervals
            time.sleep(random.uniform(2, 5))

            # Random action selection with adjusted weights to favor lighter operations
            action = random.choices(
                ['view_home', 'list_guests', 'search_rooms', 'view_reservation', 'create_guest'],
                weights=[0.2, 0.3, 0.3, 0.15, 0.05],  # Increased weight for lighter operations
                k=1
            )[0]
...
class SustainedLoadTest(LoadTestCase):
    ...
    def test_sustained_concurrent_users(self):
        ...
        num_users = 5  # Reduced number of concurrent users
        session_duration = 60  # Reduced to 1 minute for initial testing
        all_response_times = []
        ...
        # Start concurrent user sessions with staggered starts
        threads = []
        for i in range(num_users):
            thread = threading.Thread(target=run_user_session, args=(i,))
            threads.append(thread)
            # Stagger thread starts by 1-2 seconds to prevent thundering herd
